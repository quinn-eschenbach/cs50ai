...
class Sentence:
    ...
    def known_mines(self):
        ...
        """
        # if the count is equal to the length of the cells all cells are mines
    ...
    def known_safes(self):
        ...
        """
        # if count is zero we know all cells are safu
        if self.count == 0:
            return self.cells
        # else return empty set
        return set()
...
class MinesweeperAI:
    ...
    def add_knowledge(self, cell, count):
        ...
        for i in range(-1, 2):
            for j in range(-1, 2):
                if (
                    cell[0] + i >= 0
                    and cell[1] + j >= 0
                    and cell[0] + i < self.height
                    and cell[1] + j < self.width
                ):
                    new_cell = (cell[0] + i, cell[1] + j)
                    if new_cell in self.mines:
                        # if its a known mine, skip and lower the count
                        count -= 1
                    elif new_cell not in self.safes:
                        cells_for_sentence.add(new_cell)

        new_sentence = Sentence(cells=cells_for_sentence, count=count)
        ...
        while updated:
            updated = False
            new_mines = self.find_new_mines_and_safes()
            new_sentences = self.find_sentences()
    ...
    def find_sentences(self):
        ...
        for sentence_1 in self.knowledge.copy():
            for sentence_2 in self.knowledge.copy():
                ...
                if (
                    sentence_1.cells.issubset(sentence_2.cells)
                    and len(sentence_1.cells) > 0
                ):
    ...
    def find_new_mines_and_safes(self):
        updated = False
    ...
    def make_random_move(self):
        ...
        """
        height_range = list(range(self.height))
        width_range = list(range(self.width))

        random.shuffle(height_range)
